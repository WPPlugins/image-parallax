...
class Image_Parallax {
	...
	static function get_layers($parallax_ID) {
		...
		if (count($images) > 0) {
			...
			$width = $src[1];
			$height = $src[2];
			$layers .= '><div class="parallax-container"><div class="parallax-layers intensity-'.$intensity.($vertical ? ' vertical' : '').($horizontal ? ' horizontal' : '').'" style="width:'.$width.'px;height:'.$height.'px">';
